...
export interface PelaksanaanFilterRequest {
  page: number;
  size: number;
  search?: string;
  judul?: string;
...
interface PelaksanaanFiltersProps {
  onFilterChange: (filters: PelaksanaanFilterRequest) => void;
  currentFilters: PelaksanaanFilterRequest;
  viewMode?: string;
...
export default function PelaksanaanFilters({ 
  onFilterChange, 
  currentFilters, 
  viewMode = "grid",
  onViewModeChange,
  ...
  totalItems = 0,
  currentItems = 0
}: PelaksanaanFiltersProps) {
  const [localFilters, setLocalFilters] = useState<PelaksanaanFilterRequest>(currentFilters);
  const [showFilters, setShowFilters] = useState(false);

  const handleInputChange = (key: keyof PelaksanaanFilterRequest, value: string) => {
  ...
  const clearFilters = () => {
    const defaultFilters: PelaksanaanFilterRequest = {
      page: 0,
      ...
      <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-3 sm:p-4">
        <div className="space-y-3 sm:space-y-0 sm:flex sm:items-center sm:justify-between">
          ...
          <div className="text-sm text-gray-600 dark:text-gray-400 text-center sm:text-left">
            Showing <span className="font-medium text-gray-900 dark:text-white">{currentItems}</span> of <span className="font-medium text-gray-900 dark:text-white">{totalItems}</span> pelaksanaan
          </div>
        ...
        <Card className="max-w-full overflow-hidden">
          <CardHeader className="pb-4 px-3 sm:px-6">
            <CardTitle className="text-base sm:text-lg flex items-center gap-2">
              ...
              <span className="hidden sm:inline">Filter Pelaksanaan Kegiatan</span>
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="space-y-2">
                  <Label htmlFor="tanggalSelesaiFrom" className="text-xs sm:text-sm">Mulai dari</Label>
                  <Input
                    id="tanggalSelesaiFrom"
                    type="date"
                    value={localFilters.tanggalSelesaiFrom || ""}
                    onChange={(e) => handleInputChange("tanggalSelesaiFrom", e.target.value)}
                    className="w-full text-sm"
                  />
                </div>
                <div className="space-y-2">
                  <Label htmlFor="tanggalSelesaiTo" className="text-xs sm:text-sm">Sampai</Label>
                  <Input
                    id="tanggalSelesaiTo"
                    type="date"
                    value={localFilters.tanggalSelesaiTo || ""}
                    onChange={(e) => handleInputChange("tanggalSelesaiTo", e.target.value)}
                    className="w-full text-sm"
                  />
                </div>
              </div>
